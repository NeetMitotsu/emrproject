...
//        combo.getComponent(0).addMouseListener(new TypeMouseListener(table, combo));
	    ...
	    /* 生理病理下拉列表 */
        final JComboBox shengliBingliComboBox = new JComboBox();
        for (TypeColor tc : TypeColorMap.getShengliBingliTypeArray()) {
            shengliBingliComboBox.addItem(tc);
        }
        shengliBingliComboBox.setRenderer(new ComboxRender(true));
        shengliBingliComboBox.setEditable(false);
